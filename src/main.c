...
void drawDataPoints ( double data [ ] , uint8_t stop_position ) {
  ...
  switch ( flags & SWITCH_GRAPH ) {
    case 0 :
      snprintf ( text , MAX_CHARS_IN_ROW , "BPM:   %.0f  |  %.0f" , MIN , MAX ) ; break ;
    case 1 :
      snprintf ( text , MAX_CHARS_IN_ROW , "dT:   %.2f  |  %.2f" , MIN , MAX )  ; break ;
    default : break ;
  }
...
int main ( void ) {
  ...
  while ( 1 ) {
    ...
    if ( ( flags & CLEAR_EN ) && LM75_accessed ) {
      flags &= ~CLEAR_EN ;
      ssd1306_clearScreen ( ) ;
      if ( !( flags & ( SWITCHED | MENU_CHECKER ) ) ) { drawBoundary ( ) ; }
    }
        ...
        if ( LM75_accessed ) {
          switch ( flags & MENU_CHECKER ) {
            case 0 :
              ...
              if ( flags & ( SNAPSHOT_EN | FIRST_PULSE_MEASURED ) ) {
                flags &= ~SNAPSHOT_EN ;
